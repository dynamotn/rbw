...
async fn read_master_password(
    ...
    if let Some(cmd) = config_master_password_command().await? {
        ...
        let mut opts = tokio::process::Command::new(shell);
        ...
        let mut child = opts
            .spawn()
            .map_err(|source| rbw::error::Error::Spawn { source })?;
        ...
        if !status.success() {
            anyhow::bail!(
                "master_password_command exited with an error: {:#}",
                status
            );
        }
